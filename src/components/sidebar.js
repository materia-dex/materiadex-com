...
const SideBar = props => {
  const data = useStaticQuery(graphql`
    query {
      topNavDocsMateria: allDirectory(
        filter: { sourceInstanceName: { eq: "docs" }, relativeDirectory: { eq: "materia" } }
      ...
      docsMateria: allMdx(
        filter: { fileAbsolutePath: { regex: "/docs/materia/" } }
      ...
      materiaReference: allFile(filter: { absolutePath: { regex: "/docs/materia/" }, ext: { eq: ".js" } }) {
        edges {
          node {
            name
            absolutePath
  ...
  const materiaToggle = props.path.includes('/docs/materia')

  const navData = data.topNavDocsMateria
  const listData = data.docsMateria

  const atTopLevel =
    props.path === '/docs/v1/' || props.path === '/docs/materia/' || props.path === '/docs/materia' || props.path === '/docs/v1'
      ...
      <ListWrapper>
        {atTopLevel ? (
          <StyledLink
            ...
            to={`/docs/${materiaToggle ? 'materia' : 'v1'}/`}
          >
            Introduction
          </StyledLink>
        ) : (
            ...
            return (
              !hideRender && (
                <CollapsibleList
                  ...
                  node={node}
                  listData={listData}
                  referenceData={data.materiaReference}
                  path={props.path}
                  parent={props.parent}
                  atTopLevel={atTopLevel}
                  topLevel={materiaToggle ? '/docs/materia' : '/docs/v1'}
                />
              )
            )
          })}
        ...
        {atTopLevel && (
          <StyledList style={{ marginTop: '1rem' }}>
            ...
            <StyledListItem>
              <StyledLink to={'/docs/materia/governance/governance-reference'}>Governance</StyledLink>
            </StyledListItem>
          </StyledList>
        )}
      </ListWrapper>
