...
The subgraph can be queried to retrieve important information about Materia, pairs, tokens, transactions, users, and more. This page will provide examples for common queries.

To try these queries and run your own visit the [subgraph sandbox](https://thegraph.com/explorer/subgraph/materia-dex/materia).

### Global Data

To query global data you can pass in the Materia Factory address and select from available fields.
...
```
{
 MateriaFactory(id: "0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f"){
   totalVolumeUSD
   totalLiquidityUSD
   txCount
 }
...
```
{
 MateriaFactory(id: "0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f", block: {number: 10291203}){
   totalVolumeUSD
   totalLiquidityUSD
   txCount
 }
...
#### All pairs in Materia

The Graph limits entity return amounts to 1000 per query as of now. To get all pairs on Materia use a loop and graphql skip query to fetch multiple chunks of 1000 pairs. The query would look like this (where skip is some incrementing variable passed into your query).
...
Order by liquidity to get the most liquid pairs in Materia.
...
Token data can be fetched using the token contract address as an ID. Token data is aggregated across all pairs the token is included in. Any token that is included in some pair in Materia can be queried.

#### Token Overview

Get a snapshot of the current stats on a token in Materia. This query fetches current stats on DAI.
...
You can use the Bundle entity to query current USD price of ETH in Materia based on a weighted average of stablecoins.
