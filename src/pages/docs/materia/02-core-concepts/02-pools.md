...
Materia is unique in that it doesn’t use an order book to derive the price of an asset or to match buyers and sellers of tokens. Instead, Materia uses what are called Liquidity Pools.
...
Materia focuses on the strengths of Ethereum to reimagine token swaps from first principles.
...
<!-- Enter Materia and its underlying core mechanism of Liquidity Pools.

The goal of a liquidity provider is to earn fees from making markets (enabling the trading) for some asset. Instead of requiring liquidity providers to place discrete orders onto an order book, Materia combines (or pools) all available liquidity between two tokens in an Ethereum smart contract called a Liquidity Pool.
